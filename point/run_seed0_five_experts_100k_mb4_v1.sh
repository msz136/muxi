...
run_one() {
  ...
  local output_dir="${OUT_BASE}/seed0_${expert}_100k_mb4_v1"
  local log_path="${ROOT}/logs/seed0_${expert}_100k_mb4_v1.log"
...
main() {
  echo "[run] tag=${RUN_TAG} limit=${LIMIT_SAMPLES} microbatch=4 base=${MODEL_BASE}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
