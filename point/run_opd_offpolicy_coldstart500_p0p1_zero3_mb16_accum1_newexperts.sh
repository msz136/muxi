...
RUN_NAME="opd_offpolicy_coldstart500_newexperts_p0p1_fullvocab_maxnew128_prefix64_veto1to0_zero3_mb16_accum1_$(date +%Y%m%d_%H%M%S)"
...
: > "${PEAK_LOG}"
echo "${OUT}" > /data/msz/point/latest_opd_offpolicy_coldstart500_newexperts_p0p1_output.txt
echo "${LOG}" > /data/msz/point/latest_opd_offpolicy_coldstart500_newexperts_p0p1_log.txt
...
deepspeed --num_gpus=8 train_opd_online_vl.py train \
  ...
  --opd-rollout-source teacher \
  --opd-max-new-tokens 128 \
  --opd-prefix-loss-tokens 64 \
  --opd-veto-beta-start 1.0 \
  --opd-veto-beta-end 0.0 \
  --opd-veto-beta-steps 100 \
