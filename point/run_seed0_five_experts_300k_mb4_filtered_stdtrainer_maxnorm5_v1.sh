...
RUN_TAG=seed0_300k_mb4_filtered_stdtrainer_maxnorm5_v1
LIMIT_SAMPLES=${LIMIT_SAMPLES:-300000}
MAX_GRAD_NORM=5.0
...
cat > "${ROOT}/outputs/${RUN_TAG}_manifest.json" <<EOF
{
  ...
  "expected_steps_per_expert": 2344,
...
run_one() {
  ...
  local output_dir="${OUT_BASE}/seed0_${expert}_300k_mb4_filtered_stdtrainer_maxnorm5_v1"
  local log_path="${ROOT}/logs/seed0_${expert}_300k_mb4_filtered_stdtrainer_maxnorm5_v1.log"
  ...
  echo "[start] $(date '+%F %T') expert=${expert} mb=4 max_grad_norm=${MAX_GRAD_NORM} data=${data_path} output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
  ...
  echo "[done] $(date '+%F %T') expert=${expert} mb=4 max_grad_norm=${MAX_GRAD_NORM} output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
}

main() {
  echo "[run] tag=${RUN_TAG} limit=${LIMIT_SAMPLES} mb=4 max_grad_norm=${MAX_GRAD_NORM} base=${MODEL_BASE}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
