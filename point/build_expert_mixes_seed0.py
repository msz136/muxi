...
def split_for_image(row: dict[str, Any], source: str, seed: int) -> str:
    ...
    value = stable_int(f"{source}\t{key}", seed) % EVAL_HASH_MOD
        ...
        for split, bucket in (("train", train_images), ("eval", eval_images)):
            with (out_dir / expert / f"{split}.jsonl").open("r", encoding="utf-8") as f:
                for line in f:
                    ...
                    images = row.get("image") or []
                    if images:
                        bucket.add(str(images[0]))
                        if not Path(str(images[0])).exists():
                            missing_media[split] += 1
                    if not row.get("conversations") or (("<box>" in json.dumps(row, ensure_ascii=False) or "<point>" in json.dumps(row, ensure_ascii=False)) and extract_box_or_point(row) is None):
                        bad_format[split] += 1
        report[expert] = {
