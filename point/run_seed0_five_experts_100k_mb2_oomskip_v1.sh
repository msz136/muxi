...
RUN_TAG=seed0_100k_mb2_oomskip_v1
...
cat > "${ROOT}/outputs/${RUN_TAG}_manifest.json" <<EOF
{
  ...
  "expected_steps_per_expert": 1563,
  "save_policy": "final_model_only_no_intermediate_checkpoints",
  "trainer_policy": "minimal_oom_skip_trainer",
...
run_one() {
  ...
  local output_dir="${OUT_BASE}/seed0_${expert}_100k_mb2_oomskip_v1"
  local log_path="${ROOT}/logs/seed0_${expert}_100k_mb2_oomskip_v1.log"
  ...
  echo "[start] $(date '+%F %T') expert=${expert} mb=2 data=${data_path} output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
  ...
  echo "[done] $(date '+%F %T') expert=${expert} mb=2 output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
}

main() {
  echo "[run] tag=${RUN_TAG} limit=${LIMIT_SAMPLES} mb=2 base=${MODEL_BASE}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
