...
RUN_TAG=seed0_200k_v1
LIMIT_SAMPLES=${LIMIT_SAMPLES:-200000}
...
run_one() {
  ...
  local output_dir="${OUT_BASE}/seed0_${expert}_200k_v1"
  local log_path="${ROOT}/logs/seed0_${expert}_200k_v1.log"
  ...
  echo "[start] $(date '+%F %T') expert=${expert} data=${data_path} output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
...
main() {
  echo "[run] tag=${RUN_TAG} limit=${LIMIT_SAMPLES} base=${MODEL_BASE}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
