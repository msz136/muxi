...
RUN_TAG=seed0_100k_mb1_filtered_stdtrainer_maxnorm5_v1
...
run_one() {
  ...
  local output_dir="${OUT_BASE}/seed0_${expert}_100k_mb1_filtered_stdtrainer_maxnorm5_v1"
  local log_path="${ROOT}/logs/seed0_${expert}_100k_mb1_filtered_stdtrainer_maxnorm5_v1.log"
  ...
  echo "[start] $(date '+%F %T') expert=${expert} mb=1 max_grad_norm=${MAX_GRAD_NORM} data=${data_path} output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
  ...
  echo "[done] $(date '+%F %T') expert=${expert} mb=1 max_grad_norm=${MAX_GRAD_NORM} output=${output_dir}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
}

main() {
  echo "[run] tag=${RUN_TAG} limit=${LIMIT_SAMPLES} mb=1 max_grad_norm=${MAX_GRAD_NORM} base=${MODEL_BASE}" | tee -a "${ROOT}/logs/${RUN_TAG}.log"
